...
void draw_dda_fp_at(int ray_index, int num_rays, Vector4 v, int screen_w, int screen_h, float sx, float sy)
{
    ...
    float thickness = col_w + 1.0f;
    float col_x_center = sx + (ray_index * col_w) + (col_w / 2.0f);

    float wall_h = (64.0f * screen_h) / v.z;

    float wall_top = sy + (screen_h * 0.5f) - (wall_h * 0.5f);
    float wall_bottom = sy + (screen_h * 0.5f) + (wall_h * 0.5f);

    // Ceiling
    DrawLineEx((Vector2){col_x_center, sy}, (Vector2){col_x_center, wall_top}, thickness, SKYBLUE);

    // Floor
    DrawLineEx((Vector2){col_x_center, wall_bottom}, (Vector2){col_x_center, sy + screen_h}, thickness, DARKGRAY);

    // Wall column
    Color c = (v.w == 0) ? BLUE : (Color){0, 0, 150, 255};
    DrawLineEx((Vector2){col_x_center, wall_top}, (Vector2){col_x_center, wall_bottom}, thickness, c);
}
//[=======================================================================================================================================]
//  DDA 
//[=======================================================================================================================================]
...
void manage_scissor_camera(Rectangle *scissor_rect, Camera2D *cam, int *map, Vector2 subdivisions, Player *player, Color c, Vector2 world_tile_size, int id, Vector4 *rdda)
{
    ...
    case (RENDER):
    {
        ...
        case (TREE):
        {
            ...
            current_offset_y += 20;
            DrawText(TextFormat("x: %.2f, y: %.2f", player->position.x, player->position.y), current_offset_x, current_offset_y, font_size, WHITE);
            // Draw Player Angle
            current_offset_y += 20;
